...
haplotaggeR <- function(haplotag.bams.path, chromosomes = paste0("chr", c(1:22, "X")), output = NULL) {
  ...
  for (i in 1:length(haplotag.bams)) {
    ...
    message("Processing bamfile for cell ", cell.id, " ...")
    ...
    message("Processing bamfile for cell ", cell.id, " done!")
  }
...
plot_3channel_chr <- function(cell_ID, roi = NULL, plot_range = NULL, chromosome = NULL) {
  ...
  plt1 <- ggplot(in.d) +
    ...
    ylab("W:C ratio")+
    scale_x_reverse(breaks = pretty_breaks(15), labels = format_Mb) +
  ...
  plt3 <- ggplot(in.d.hap1) +
    ...
    geom_linerange(data=in.d.hap1[in.d.hap1$w!=0,], aes(ymin=0, ymax=-w), size=.5, color="red", alpha = 0.33) + # SNPs on W reads (right)
    geom_linerange(data=in.d.hap1[in.d.hap1$c!=0,], aes(ymin=0, ymax=c),  size=.5, color="red", alpha = 0.33) + # SNPs on C reads (left)
  ...
  plt4 <- ggplot(in.d.hap2) +
    aes(x = ((start+end)/2)) +
    ### REPLACED WITH LINE+BALL 
    geom_linerange(data=in.d.hap2[in.d.hap2$w!=0,], aes(ymin=0, ymax=-w), size=.5, color="blue", alpha = 0.33) + # SNPs on W reads (right)
    geom_linerange(data=in.d.hap2[in.d.hap2$c!=0,], aes(ymin=0, ymax=c), size=.5, color="blue", alpha = 0.33) + # SNPs on C reads (left)
