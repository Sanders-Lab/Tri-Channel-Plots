...
haplotaggeR <- function(haplotag.bams.path, chromosomes = paste0("chr", c(1:22, "X", "Y")), output = NULL) {
...
plot_channels <- function(cell_ID, plot_range = NULL, channels = c(4,3,2,1), chromosome = NULL, roi = NULL) {
  ...
  if (channels > 4) {
    stop("Abort mission! Channels must be integer of 4 or smaller!")
  } else {
  ...
  #     message("Fetching haplotype data for cell ", cell_ID, " on ", chromosome, " ...")
  #     in.d.hap <- d.hap %>% 
  #       filter(cell == cell_ID & chrom == chromosome & end %in% plot_range)
  #     
  #     #filter out roi data here depending on what youre analysing (remove/add arguments as needed)
  #     message("Fetching count data for ", cell_ID, " on ", chromosome, " ...")
  #     ind <- d %>% 
  ...
  #   } else {
  #     if (channels == 3) {
  #       tri_plot <- plot_grid(plt1,plt2,plt5, nrow = 1, align = "v")
  #       return(tri_plot)
  #     } else {
  #       if (channels == 4) {
